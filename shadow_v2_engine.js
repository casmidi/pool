...
/**
 * Get adaptive close advice for a specific pool position.
 * Reads shadow_v2_summary.json for today's adaptive performance.
 * If the best route is wait_5m_recheck with positive impact, advises delay.
 * Also checks pool-specific data in shadow_v2_cases.json.
 *
 * @param {string} poolAddress - Pool address to check
 * @param {string} [poolName] - Pool name for logging
 * @param {object} [closeRule] - The deterministic close rule (e.g. {rule: 4, reason: "OOR"})
 * @returns {{ shouldDelay: boolean, delayMinutes: number, reason: string }}
 */
export function getAdaptiveCloseAdvice(poolAddress, poolName, closeRule) {
  try {
    // Only applies to OOR rules (rule 4)
    if (!closeRule || closeRule.rule !== 4) {
      return { shouldDelay: false, delayMinutes: 0, reason: "not_oor_rule" };
    }

    // Read today's summary
    const today = dateKey();
    const store = readJson(SUMMARY_PATH, { summaries: [] });
    const summaries = Array.isArray(store.summaries) ? store.summaries : [];
    const todaySummary = summaries.find((s) => s.date === today);

    if (!todaySummary) {
      return { shouldDelay: false, delayMinutes: 0, reason: "no_today_summary" };
    }

    // Check adaptive data
    const bestRoute = todaySummary.adaptive_best_route || "none";
    const bestImpact = num(todaySummary.adaptive_best_impact_sol, 0);
    const totalImpact = num(todaySummary.adaptive_impact_sol, 0);

    // Only advise delay if wait_5m_recheck is the best route with positive impact
    if (bestRoute !== "wait_5m_recheck" || bestImpact <= 0.1) {
      return {
        shouldDelay: false,
        delayMinutes: 0,
        reason: `adaptive_best=${bestRoute} impact=${bestImpact.toFixed(4)}`,
      };
    }

    // Check pool-specific cases for extra context
    const cases = readJson(CASES_PATH, { cases: [] });
    const poolCases = (cases.cases || []).filter(
      (c) => String(c.pool_address).toLowerCase() === String(poolAddress).toLowerCase()
    );

    // If this pool has a good track record with adaptive, be more confident
    const poolHasGoodHistory = poolCases.some((c) => {
      const variants = c.adaptive_shadow?.variants || {};
      const waitVar = variants.wait_5m_recheck;
      return waitVar && waitVar.outcome === "ADAPTIVE_WIN";
    });

    const delayMinutes = poolHasGoodHistory ? 5 : 3;
    const poolNote = poolName ? ` for ${poolName}` : "";

    return {
      shouldDelay: true,
      delayMinutes,
      reason: `adaptive_wait_5m: best_route=${bestRoute} impact=${bestImpact.toFixed(4)} SOL total=${totalImpact.toFixed(4)} SOL${poolHasGoodHistory ? " pool_has_win_history" : ""}${poolNote}`,
    };
  } catch (err) {
    return { shouldDelay: false, delayMinutes: 0, reason: `error: ${err.message}` };
  }
}
