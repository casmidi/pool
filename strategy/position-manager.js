...
export const DEFAULT_POSITION_RULES = {
  ...
  applyIntelligenceDecayRules: false,
  maxDecayRiskToHold: 75,
...
export function evaluatePosition(
  ...
  const decayAnalysis = predictPoolDecay(scoredPool ?? position, [], r.decayOptions ?? {});
  const summary = () => ({
    holdingDays,
    ilFraction,
    pnlUsd,
  ...
  if (r.applyIntelligenceDecayRules && decayAnalysis.decay_risk >= r.maxDecayRiskToHold && action === "HOLD") {
    action  = "EXIT";
    urgency = "medium";
    reasons.push(`decay risk ${decayAnalysis.decay_risk} >= max ${r.maxDecayRiskToHold}`);
  }
  ...
  return buildResult(action, urgency, reasons, summary());
}
