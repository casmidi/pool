...
export function evaluateAntiOorRangeAdaptation(plan = {}, antiOor = {}, options = {}) {
  const isSingleSidedSol = options.isSingleSidedSol !== false;
  const minBins = Math.max(35, num(options.minBinsBelow ?? options.config?.strategy?.minBinsBelow, 35));
  const maxBins = Math.max(minBins, num(options.maxBinsBelow ?? options.config?.strategy?.maxBinsBelow, 69));
  const rec = antiOor.dynamicRangeWidth?.recommendation || antiOor.recommendation || "KEEP_STANDARD_RANGE";
  const multiplier = num(antiOor.dynamicRangeWidth?.widthMultiplier ?? antiOor.widthMultiplier, 1);
  const currentBelow = num(plan.bins_below, minBins);
  const currentAbove = num(plan.bins_above, 0);
  const suggestedWidth = num(antiOor.dynamicRangeWidth?.suggestedWidthBins, null);
  const widenedBelow = Math.round(clamp(
    suggestedWidth !== null ? suggestedWidth : currentBelow * multiplier,
    minBins,
    maxBins,
  ));

  if (isSingleSidedSol && rec === "WIDEN_AND_SHIFT_UP") {
    return {
      recommendation: rec,
      legal: true,
      final_range_action: widenedBelow > currentBelow ? "WIDEN_RANGE_UPWARD_SAFETY" : "KEEP_STANDARD_RANGE",
      bins_below: widenedBelow,
      bins_above: 0,
      reason: "single-side SOL cannot shift up; widening range downward as safety buffer instead",
    };
  }

  if (isSingleSidedSol && rec === "WIDEN_AND_SHIFT_DOWN") {
    return {
      recommendation: rec,
      legal: true,
      final_range_action: widenedBelow > currentBelow ? "WIDEN_RANGE_DOWNWARD" : "KEEP_STANDARD_RANGE",
      bins_below: widenedBelow,
      bins_above: 0,
      reason: "single-side SOL can safely widen downside range without using bins_above",
    };
  }
  ...
  if (rec === "WIDEN_RANGE") {
    return {
      recommendation: rec,
      legal: true,
      final_range_action: widenedBelow > currentBelow ? "WIDEN_RANGE" : "KEEP_STANDARD_RANGE",
      bins_below: widenedBelow,
      bins_above: isSingleSidedSol ? 0 : currentAbove,
      reason: "range widened within configured safe limits",
    };
  }
  ...
  return {
    recommendation: rec,
    legal: true,
    final_range_action: "KEEP_STANDARD_RANGE",
    bins_below: currentBelow,
    bins_above: isSingleSidedSol ? 0 : currentAbove,
    reason: "no dynamic range adjustment required",
  };
}
