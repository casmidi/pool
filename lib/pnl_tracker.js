...
import { appendTradeReplayEvent, generateIncidentReport } from "./operator_intelligence.js";
import { createDecisionSnapshot, createSourceTruth, detectSignalLoss } from "./source_truth.js";
import {
  createForensicTraceId,
  recordForensicDeploy,
  recordForensicExit,
  recordForensicRuntime,
} from "./forensic_scanner.js";
...
export function recordDeploy({
  ...
  feeVelocityBoost = null,
}) {
  const store = load();
  const id = `${Date.now()}_${Math.random().toString(36).slice(2, 6)}`;
  const forensicTraceId = createForensicTraceId(poolName ?? poolAddress ?? id);
  ...
  const entryTruth = createSourceTruth(truthParams);
  const decisionSnapshot = createDecisionSnapshot(truthParams);
  const trade = {
    id,
    ...
    source_wallet_rank: sourceWalletRank ?? null,
    source_wallet_score: sourceWalletScore ?? null,
    source_wallet_grade: sourceWalletGrade ?? null,
    source_wallet_type: sourceWalletType ?? null,
    source_wallet_confidence: sourceWalletConfidence ?? null,
    copy_engine_source: copyEngineSource ?? deployArgs?.source ?? null,
    source_signal_id: sourceSignalId ?? deployArgs?.source_signal_id ?? deployArgs?.signal_id ?? null,
    decision_confidence: decisionConfidence ?? null,
    decision_breakdown: decisionBreakdown ?? null,
    entry_truth: entryTruth,
    decision_snapshot: decisionSnapshot,
    truth_snapshot_hash: decisionSnapshot.snapshotHash,
    shadow_decision: shadowDecision ?? null,
    market_regime: marketRegime ?? null,
    capital_protection: capitalProtection ?? null,
    pool_trust: poolTrust ?? null,
    alpha_edge: alphaEdge ?? null,
    alpha_rank: alphaEdge?.alphaRank ?? null,
    alpha_score: alphaEdge?.alphaScore ?? null,
    master_strategy: masterStrategy ?? null,
  ...
  trade.signal_loss = detectSignalLoss(trade);
  store.trades.push(trade);
  save(store);
  recordForensicDeploy(trade);
  appendTradeReplayEvent(trade, {
    type: "deploy",
    amountSol: trade.amount_sol,
    confidence: trade.decision_confidence,
    organic: trade.organic_score,
    regime: trade.market_regime?.regime ?? null,
    trust: trade.pool_trust?.score ?? null,
  });
  return id;
}
 ...
 */
export function recordClose({ poolAddress, positionAddress, poolName, pnlPct, pnlUsd, feesUsd, closeReason, solPrice, exitBin, exitPrice, exitSide }) {
  const store = load();
  ...
  if (trade) {
    ...
    trade.exit_bin     = exitBin ?? trade.active_bin ?? null;
    trade.exit_price   = exitPrice ?? trade.current_price ?? null;
    trade.exit_side    = exitSide ?? trade.out_of_range_side ?? null;
    Object.assign(trade, computeCloseMetrics(trade));
    trade.status       = "closed";
    recordForensicExit(trade);
    appendTradeReplayEvent(trade, {
      type: "close",
      ...
      minutesOutOfRange: trade.minutes_out_of_range ?? 0,
    });
    generateIncidentReport(trade);
  }
...
function computeCloseMetrics(trade) {
  const amountSol = _num(trade.amount_sol, 0);
  const pnlSol = _num(trade.pnl_sol);
  const pnlPct = _num(trade.pnl_pct);
  const feesUsd = _num(trade.fees_usd);
  const deployMs = new Date(trade.deploy_time || 0).getTime();
  const closeMs = new Date(trade.close_time || 0).getTime();
  const minutesHeld = Number.isFinite(deployMs) && Number.isFinite(closeMs)
    ? Math.max(0, (closeMs - deployMs) / 60000)
    : _num(trade.minutes_held, null);
  const feeSol = feesUsd != null && _solPriceCache > 0 ? feesUsd / _solPriceCache : _num(trade.paper_fee_sol, null);
  const feeApr = amountSol > 0 && feeSol != null && minutesHeld > 0
    ? (feeSol / amountSol) * (525600 / minutesHeld) * 100
    : null;
  const ilProxySol = pnlSol != null && feeSol != null ? Math.max(0, feeSol - pnlSol) : null;
  const feeVsIlRatio = ilProxySol != null && ilProxySol > 0 && feeSol != null
    ? feeSol / ilProxySol
    : (feeSol != null && feeSol > 0 ? 999 : null);
  const lower = _num(trade.lower_bin);
  const upper = _num(trade.upper_bin);
  const entry = _num(trade.entry_bin ?? trade.active_bin);
  const exit = _num(trade.exit_bin ?? trade.active_bin);
  const width = lower != null && upper != null ? Math.max(1, upper - lower) : null;
  const drift = entry != null && exit != null ? Math.abs(exit - entry) : null;
  const rangeEfficiency = width != null && drift != null
    ? _round(Math.max(0, Math.min(100, (1 - (drift / Math.max(1, width))) * 100)), 2)
    : null;

  return {
    minutes_held: minutesHeld != null ? Math.floor(minutesHeld) : trade.minutes_held,
    realized_fee_apr_pct: feeApr != null ? _round(feeApr, 2) : null,
    fee_vs_il_ratio: feeVsIlRatio != null ? _round(feeVsIlRatio, 4) : null,
    estimated_il_sol: ilProxySol != null ? _round(ilProxySol, 9) : null,
    range_efficiency_pct: rangeEfficiency,
    expectancy_sol: pnlSol != null ? pnlSol : (pnlPct != null && amountSol > 0 ? _round((pnlPct / 100) * amountSol, 9) : null),
  };
}
...
async function _markDryRunTrade(trade, now) {
  ...
  const peakPnlPct = Math.max(Math.min(priorPeakPct, maxPnlPct * 100), pnlPct);
...
export async function simulateDryRunCloses({
  ...
  for (const t of store.trades) {
    ...
    if (t.out_of_range_side && !t.forensic_oor_event_logged) {
      const entryBin = _num(t.entry_bin);
      const activeBin = _num(t.active_bin);
      const binStep = _num(t.bin_step, 80);
      const binDelta = entryBin != null && activeBin != null ? activeBin - entryBin : null;
      recordForensicRuntime(t.forensic_trace_id, t, {
        event: t.out_of_range_side === "above" ? "BREAKOUT_UP" : "BREAKOUT_DOWN",
        pct_move: binDelta != null ? _round((Math.pow(1 + (binStep / 10000), binDelta) - 1) * 100, 2) : null,
        minutes_since_entry: Math.floor(ageMinutes),
        oor_classification: t.oor_classification ?? null,
        oor_action: t.oor_action ?? null,
        oor_reason: t.oor_reason ?? null,
      });
      t.forensic_oor_event_logged = true;
    }
    touched = true;
    ...
    t.exit_bin     = t.active_bin ?? null;
    t.exit_price   = t.current_price ?? null;
    t.exit_side    = t.out_of_range_side ?? null;
    Object.assign(t, computeCloseMetrics(t));
    t.status       = "closed";
    t.minutes_held = Math.floor(ageMinutes);
    recordForensicExit(t);
...
export function setOorClassification(positionAddress, classification) {
  const store = load();
  const trade = store.trades.find(t => t.position_address === positionAddress && t.status === "open");
  if (!trade) return false;
  trade.oor_classification = classification.classification ?? null;
  trade.oor_action = classification.action ?? null;
  trade.oor_reason = classification.reason ?? null;
  save(store);
  return true;
}

/**
 * Hitung jumlah kerugian beruntun dari closed trades (semua source).
 * Untuk airbag Alpha Conviction — turunkan size jika rugi terus.
 */
export function getConsecutiveLosses() {
  const store = load();
  const closed = store.trades
    .filter(t => t.status === "closed" && t.pnl_sol != null)
    .sort((a, b) => new Date(b.close_time || 0) - new Date(a.close_time || 0));
  let count = 0;
  for (const t of closed) {
    if (Number(t.pnl_sol) < 0) count++;
    else break;
  }
  return count;
}
...
export function formatSummaryTelegram(isDryRun = true) {
  ...
  const pnlColor = s.net_pnl_sol >= 0 ? "🟢" : "🔴";
  const heroPct = `${sign}${s.net_pnl_pct}%`;
  const heroSol = `${sign}${s.net_pnl_sol} SOL`;
  const lines = [
    `📊 <b>P&amp;L Summary${tag}</b>`,
    solPriceLine,
    ``,
    `${pnlColor} <b>${heroPct}</b>  ◎ ${heroSol}`,
    `  Balance: ${s.current_sol} SOL  (awal ${s.initial_sol} SOL)`,
    s.total_pnl_usd ? `  PnL USD: ${sign}$${s.total_pnl_usd}` : "",
    (s.ai_cost_usd || s.tx_cost_usd) ? `  Costs: AI $${s.ai_cost_usd} | tx est. $${s.tx_cost_usd}` : "",
    ``,
    `📈 <b>${s.total_trades}</b> trade  |  ✅ <b>${s.wins}</b> win  ❌ <b>${s.losses}</b> loss  |  <b>${s.win_rate}%</b> WR`,
    `  Terbuka: ${s.open_positions}  |  Avg PnL: ${s.avg_pnl_pct >= 0 ? "+" : ""}${s.avg_pnl_pct}%`,
    s.total_fees_usd ? `  Fees earned: $${s.total_fees_usd}` : "",
    ``,
...
/**
 * Format PnL Calendar untuk Telegram.
 * Menampilkan PnL per hari dalam sebulan dengan total menonjol di atas.
 */
export function formatPnLCalendar(isDryRun = true, monthOffset = 0) {
  const store  = load();
  const tag    = isDryRun ? " [DRY RUN]" : "";
  const closed = store.trades.filter(t => t.status === "closed");
  ...
  const monthLabel = `${monthNames[targetMonth.getMonth()]} ${targetMonth.getFullYear()}`;
  const monthPrefix = targetMonth.getFullYear() + "-" + String(targetMonth.getMonth() + 1).padStart(2, "0");

  const daily = {};
  const dailyCounts = {};
  for (const t of closed) {
    const day = _dateKey(t.close_time || t.deploy_time);
    if (!day || !day.startsWith(monthPrefix)) continue;
    const pnlSol = t.pnl_sol != null ? t.pnl_sol : ((t.pnl_pct ?? 0) / 100) * (t.amount_sol ?? 0);
    daily[day] = (daily[day] || 0) + pnlSol;
    if (!dailyCounts[day]) dailyCounts[day] = { wins: 0, losses: 0 };
    if ((t.pnl_pct ?? 0) > 0) dailyCounts[day].wins++;
    else dailyCounts[day].losses++;
  }

  const days = Object.keys(daily).sort();
  const values = Object.values(daily);
  const total = values.reduce((s, v) => s + v, 0);
  const best = days.length ? Math.max(...values) : 0;
  const worst = days.length ? Math.min(...values) : 0;
  const winDays = days.filter(d => daily[d] > 0).length;
  const lossDays = days.filter(d => daily[d] < 0).length;
  const zeroDays = days.filter(d => daily[d] === 0).length;
  const sign = total >= 0 ? "+" : "";
  const solPrice = _solPriceCache || 0;

  const lines = [
    `📅 <b>PnL Calendar${tag}</b>`,
    solPrice > 0 ? `◎ SOL: <b>$${solPrice}</b>` : "",
    ``,
    `${total >= 0 ? "🟢" : "🔴"} <b>${sign}${total.toFixed(4)} SOL</b>`,
    `  ${monthLabel} — Total PnL`,
    ``,
  ];
  ...
  const dayLines = days.map(d => {
    ...
    const parts = [];
    if (c.wins) parts.push(`✅${c.wins}x`);
    if (c.losses) parts.push(`❌${c.losses}x`);
    const countStr = parts.length ? ` ${parts.join(" ")}` : "";
    return `${d.slice(5)}  ${icon} ${val >= 0 ? "+" : ""}${val.toFixed(4)} SOL${countStr}`;
  });
  lines.push(...dayLines);
  ...
  lines.push(``);
  lines.push(`📊 Trading days: ${totalDays}  |  🟢 ${winDays} win  🔴 ${lossDays} loss  ⚪ ${zeroDays} zero`);
  lines.push(`🏆 Best: <b>+${best.toFixed(4)} SOL</b>  |  💀 Worst: <b>${worst.toFixed(4)} SOL</b>`);
