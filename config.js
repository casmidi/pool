...
if (u.bitqueryApiKey) process.env.BITQUERY_API_KEY ||= u.bitqueryApiKey;
if (u.redisUrl)      process.env.REDIS_URL       ||= u.redisUrl;
if (u.redisPrefix)   process.env.REDIS_PREFIX    ||= u.redisPrefix;
...
export const config = {
  ...
  screening: {
    ...
    maxDeployVolatility: u.maxDeployVolatility ?? 2.5,
    ...
    maxOorRatioForRedeploy: u.maxOorRatioForRedeploy ?? 0.6,
    ...
    minPoolWinRate: u.minPoolWinRate ?? 50,
    ...
    // Probe mode: for borderline candidates (score within 15 of minPoolScore),
    // deploy a small probe position instead of hard-blocking.
    // This lets the bot test marginal pools with minimal risk.
    probeModeEnabled: u.probeModeEnabled ?? true,
    probeSizeSol: u.probeSizeSol ?? 0.2,
    ...
    fallbackRiskEnabled: u.fallbackRiskEnabled ?? true,
    failOpenOnRiskDataUnavailable: u.failOpenOnRiskDataUnavailable ?? undefined,
    failClosedOnMissingRiskMetrics: u.failClosedOnMissingRiskMetrics ?? undefined,
  },
  ...
  management: {
    ...
    lossTriggeredCooldown:      u.lossTriggeredCooldown      ?? true,
  ...
  shadowV2Guard: {
    enabled: u.shadowV2GuardEnabled ?? true,
    enforce: u.shadowV2GuardEnforce ?? true,
    hardBlockLevels: u.shadowV2HardBlockLevels ?? ["HIGH", "CRITICAL"],
    hardBlockExitRoutes: u.shadowV2HardBlockExitRoutes ?? ["NO_ROUTE", "UNSTABLE"],
    watchPenalty: u.shadowV2WatchPenalty ?? 8,
    highPenalty: u.shadowV2HighPenalty ?? 18,
    criticalPenalty: u.shadowV2CriticalPenalty ?? 35,
    thinRoutePenalty: u.shadowV2ThinRoutePenalty ?? 6,
    unstableRoutePenalty: u.shadowV2UnstableRoutePenalty ?? 18,
    noRoutePenalty: u.shadowV2NoRoutePenalty ?? 35,
    maxPenalty: u.shadowV2MaxPenalty ?? 40,
  },
  ...
  aiOptimization: {
    enabled: u.aiOptimizationEnabled ?? true,
    lowConfidenceSkip: u.aiLowConfidenceSkip ?? 0.45,
    highConfidenceBypass: u.aiHighConfidenceBypass ?? 0.75,
    signalCacheEnabled: u.aiSignalCacheEnabled ?? true,
    signalCacheTtlMinutes: u.aiSignalCacheTtlMinutes ?? 15,
    signalCacheMaxDiffPct: u.aiSignalCacheMaxDiffPct ?? 0.05,
    cooldownMinutes: u.aiCooldownMinutes ?? 10,
    conservativeRemainingPct: u.aiConservativeRemainingPct ?? 70,
    criticalRemainingPct: u.aiCriticalRemainingPct ?? 30,
    zeroAiEmergencyMode: u.aiZeroEmergencyMode ?? false,
    targetCallsPerDay: u.aiTargetCallsPerDay ?? 60,
    premiumDeployAmountSol: u.aiPremiumDeployAmountSol ?? 1.5,
    cheapModels: u.aiCheapModels ?? [
      "openrouter/free",
      "google/gemini-2.0-flash-lite:free",
      "deepseek/deepseek-chat:free",
    ],
    premiumModels: u.aiPremiumModels ?? [
      u.aiReviewModel ?? u.reviewModel ?? "anthropic/claude-haiku-4.5",
  ...
  intelligence: {
    ...
    redisUrl: u.redisUrl ?? process.env.REDIS_URL ?? "",
    redisPrefix: u.redisPrefix ?? process.env.REDIS_PREFIX ?? "meridian",
  ...
  // Observability/advisory intelligence layer. Defaults are intentionally
  // read-only so existing screening, deploy, and confidence behavior stay stable.
  intelligenceLayer: {
    enabled: u.intelligenceLayerEnabled ?? true,
    enforce: u.intelligenceLayerEnforce ?? false,
    applyScoringAdjustments: u.intelligenceLayerApplyScoringAdjustments ?? false,
    applyPositionRules: u.intelligenceLayerApplyPositionRules ?? false,
    applySizingRecommendation: u.intelligenceLayerApplySizing ?? false,
    maxWeightAdjustment: u.intelligenceLayerMaxWeightAdjustment ?? 0.25,
    decayHalfLifeDays: u.intelligenceLayerDecayHalfLifeDays ?? 14,
    minRedeployScoreGap: u.intelligenceLayerMinRedeployScoreGap ?? 12,
    minRedeployNetGain: u.intelligenceLayerMinRedeployNetGain ?? 8,
  },
  ...
  // Controls how execution_intelligence.js output affects deploy behavior.
  // "strict"   — current behavior: NO ENTRY / BLOCKED / AVOID blocks deploy (backward-compatible)
  // "advisory" — execution intelligence calculates but NEVER blocks deploy; emits warnings only
  // "disabled" — skip execution intelligence entirely
  executionIntelligenceMode: u.executionIntelligenceMode ?? "strict",

  masterStrategy: {
    enabled: u.masterStrategyEnabled ?? true,
    applyToDeploy: u.masterStrategyApplyToDeploy ?? true,
    studyBeforeDeploy: u.masterStrategyStudyBeforeDeploy ?? true,
    studyLperLimit: u.masterStrategyStudyLperLimit ?? 4,
    minSamples: u.masterStrategyMinSamples ?? 3,
    minQualityScore: u.masterStrategyMinQualityScore ?? 55,
    maxObservations: u.masterStrategyMaxObservations ?? 1500,
    observeIgnored: u.masterStrategyObserveIgnored ?? false,
  },

  backtesting: {
    enabled: u.backtestingEnabled ?? true,
    optimizerEnabled: u.backtestOptimizerEnabled ?? true,
    optimizerMaxRuns: u.backtestOptimizerMaxRuns ?? 240,
    walkForwardFolds: u.backtestWalkForwardFolds ?? 3,
    walkForwardTrainRatio: u.backtestWalkForwardTrainRatio ?? 0.6,
  },

  decision: {
    enabled: u.decisionLayerEnabled ?? true,
    enforce: u.decisionLayerEnforce ?? true,
    minScoreToCopy: u.decisionMinScoreToCopy ?? 50,
    minConfidence: u.decisionMinConfidence ?? 0.55,
    minRangeQuality: u.decisionMinRangeQuality ?? 50,
    maxVolatilityForCopy: u.decisionMaxVolatilityForCopy ?? 7,
    minFeeTvlForCopy: u.decisionMinFeeTvlForCopy ?? 0.02,
    minOrganicForCopy: u.decisionMinOrganicForCopy ?? 70,
    organicConfidenceWeight: u.decisionOrganicConfidenceWeight ?? 0.15,
    adaptiveCalibrationEnabled: u.decisionAdaptiveCalibrationEnabled ?? true,
    shadowEnabled: u.shadowDecisionEnabled ?? true,
  },

  operatorIntelligence: {
    enabled: u.operatorIntelligenceEnabled ?? true,
    capitalProtection: {
      enabled: u.capitalProtectionEnabled ?? true,
      lossTrigger: u.capitalProtectionLossTrigger ?? 3,
      winRecovery: u.capitalProtectionWinRecovery ?? 3,
      deployMultiplier: u.capitalProtectionDeployMultiplier ?? 0.5,
      confidenceBoost: u.capitalProtectionConfidenceBoost ?? 0.10,
    },
    marketRegime: {
      enabled: u.marketRegimeDetectionEnabled ?? true,
      highVolatilityThreshold: u.marketRegimeHighVolatility ?? 5,
    },
    poolTrust: {
      enabled: u.poolTrustScoreEnabled ?? true,
      softScaleBelow: u.poolTrustSoftScaleBelow ?? 50,
      hardWarnBelow: u.poolTrustHardWarnBelow ?? 40,
    },
    adaptiveConfidence: {
      enabled: u.decisionAdaptiveCalibrationEnabled ?? true,
      minSamples: u.decisionAdaptiveCalibrationMinSamples ?? 10,
    },
  },

  alphaEdge: {
    enabled: u.alphaEdgeEnabled ?? true,
    enforce: u.alphaEdgeEnforce ?? true,
    maxCrowdScore: u.alphaMaxCrowdScore ?? 90,
    minSurvivalScore: u.alphaMinSurvivalScore ?? 20,
    maxEuphoriaScore: u.alphaMaxEuphoriaScore ?? 90,
    minExpectancyScore: u.alphaMinExpectancyScore ?? 40,
    minAlphaScore: u.alphaMinAlphaScore ?? 45,
  },
  ...
  copyTrading: {
    ...
    refreshRankingWhenStale: u.copyTradingRefreshRankingWhenStale ?? true,
    maxPositionsPerWallet: u.copyTradingMaxPositionsPerWallet ?? 3,
    walletFetchTimeoutMs: u.copyTradingWalletFetchTimeoutMs ?? 20_000,
    dedupeMinutes: u.copyTradingDedupeMinutes ?? 12,
    spreadSizePct: u.copyTradingSpreadSizePct ?? null, // % of wallet per spread position (null = pakai baseAmountSol fixed)
    ...
    autoBlacklistRealized: u.copyTradingAutoBlacklistRealized ?? true,
    autoBlacklistMinSamples: u.copyTradingAutoBlacklistMinSamples ?? 3,
    autoBlacklistWinRateBelow: u.copyTradingAutoBlacklistWinRateBelow ?? 35,
    autoBlacklistPnlSolBelow: u.copyTradingAutoBlacklistPnlSolBelow ?? -0.03,
  },

  // ─── Alpha Conviction ──────────────────────
  // Concentrated copy trading: deploy large % of wallet into the single best signal.
  // Disabled by default. Set enabled: true in user-config.json to activate.
  alphaConviction: {
    enabled: u.alphaConvictionEnabled ?? false,
    sizePct: u.alphaConvictionSizePct ?? 0.18,
    minConfidence: u.alphaConvictionMinConfidence ?? 0.70,
    maxPositions: u.alphaConvictionMaxPositions ?? 1,
    minShadowWr: u.alphaConvictionMinShadowWr ?? 50,
    crowdedThreshold: u.alphaConvictionCrowdedThreshold ?? 3,
    airbagAfter: u.alphaConvictionAirbagAfter ?? 2,
    airbagFactor: u.alphaConvictionAirbagFactor ?? 0.50,
    airbagMinPct: u.alphaConvictionAirbagMinPct ?? 0.04,
  },

  autoImprovement: {
    enabled: u.autoImprovementEnabled ?? true,
    reportHourWib: u.autoImprovementReportHourWib ?? 7,
    sendTelegram: u.autoImprovementTelegram ?? true,
    windowDays: u.autoImprovementWindowDays ?? 1,
    applySuggestions: u.autoImprovementApplySuggestions ?? false,
    minClosesToTune: u.autoImprovementMinClosesToTune ?? 10,
  },
};
