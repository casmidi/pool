...
const MODEL_PRICING = [
  { match: /^openrouter\/free$/i, inputPer1M: 0, outputPer1M: 0 },
...
export function canCallAI({ model, agentType = "GENERAL" } = {}) {
  ...
  const pricing = pricingFor(model);
  const isFreeModel = pricing.inputPer1M <= 0 && pricing.outputPer1M <= 0;

  if (!isFreeModel && Number.isFinite(monthlyBudget) && monthlyBudget > 0 && month.cost_usd >= monthlyBudget) {
    month.calls = 0; month.input_tokens = 0; month.output_tokens = 0; month.total_tokens = 0; month.cost_usd = 0;
    log("ai_budget", `Monthly budget $${monthlyBudget} hit — auto-reset month bucket`);
    save(store);
  }
  if (!isFreeModel && Number.isFinite(dailyBudget) && dailyBudget > 0 && day.cost_usd >= dailyBudget) {
    day.calls = 0; day.input_tokens = 0; day.output_tokens = 0; day.total_tokens = 0; day.cost_usd = 0;
    log("ai_budget", `Daily budget $${dailyBudget} hit — auto-reset day bucket`);
    save(store);
  }
  if (!isFreeModel && Number.isFinite(maxDailyCalls) && maxDailyCalls > 0 && day.calls >= maxDailyCalls) {
    day.calls = 0; day.input_tokens = 0; day.output_tokens = 0; day.total_tokens = 0; day.cost_usd = 0;
    log("ai_budget", `Daily call cap ${maxDailyCalls} hit — auto-reset day bucket`);
    save(store);
  }
