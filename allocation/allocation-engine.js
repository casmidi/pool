...
export function runAllocation(params) {
  ...
  if (!finalCheck.allowed) {
    // Auto-downsize: if only portfolio exposure fails, reduce size to fit within cap
    const failedChecks = finalCheck.checks.filter(c => !c.passed);
    const onlyExposureFails = failedChecks.length === 1 && failedChecks[0].check.startsWith('portfolio_exposure');

    if (onlyExposureFails) {
      const profile = RISK_PROFILES[riskProfile.toUpperCase()] || RISK_PROFILES.MODERATE;
      const maxAllowed = Math.max(0, walletSolBalance * profile.maxPortfolioRisk - totalSolDeployed);
      const downsized = Math.min(sizeRec.amountSol, maxAllowed);
      const minViable = config.management?.deployAmountSol ?? 0.3;

      if (downsized >= minViable) {
        const capacity = calculateAvailableCapacity({
          walletSolBalance, openPositionCount, maxPositions, totalSolDeployed,
        });
        const finalAmount = Math.min(downsized, capacity.availableSol);
        return {
          allowed: true,
          ...
          poolVolatility,
          poolScore,
          reason: `Auto-downsized to ${finalAmount} SOL (portfolio cap ${(profile.maxPortfolioRisk * 100).toFixed(0)}%)`,
