...
export function getWalletTags(address) {
  const db = load();
  return db.wallets[address]?.tags || [];
}

export function getTaggedWallets(tagPrefix = "") {
  const db = load();
  return Object.values(db.wallets)
    .filter((w) => (w.tags || []).some((tag) => String(tag).startsWith(tagPrefix)))
    .map((w) => ({
      address: w.address,
      tags: w.tags || [],
