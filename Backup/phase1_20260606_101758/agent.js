...
function budgetAllowedModel(preferredModel, agentType, context = {}) {
  ...
  const routedModels = routeAIModels({ preferredModel, agentType, context });
  for (const candidate of uniqueModels([...routedModels, ...FREE_MODEL_CHAIN, ...MODEL_CHAIN])) {
    const budgetCheck = canCallAI({ model: candidate, agentType });
    if (budgetCheck.allowed) {
      if (candidate !== preferredModel && firstBlocked) {
        log("ai_budget_fallback", `${firstBlocked.reason} — using free fallback model ${candidate}`);
      }
...
export async function completeAI({
  ...
  const fallbackModels = uniqueModels([activeModel, ...FREE_MODEL_CHAIN, ...MODEL_CHAIN]);
...
export async function agentLoop(goal, maxSteps = config.llm.maxSteps, sessionHistory = [], agentType = "GENERAL", model = null, maxOutputTokens = null, options = {}) {
  ...
  for (let step = 0; step < maxSteps; step++) {
    ...
    try {
      ...
      const routedChain = uniqueModels([
        ...routeAIModels({ preferredModel, agentType, context: aiContext }),
        ...FREE_MODEL_CHAIN,
        ...MODEL_CHAIN,
      ]);
      let usedModel = activeModel;
