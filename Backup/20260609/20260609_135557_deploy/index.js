    ...
    try {
      ...
      if (telegramEnabled() && copySignals.length > 0) {
        const lines = copySignals.slice(0, 5).map((signal, i) => {
          ...
          const status = signal.deployResult?.blocked ? "\u274C " + (signal.deployResult.reason || "blocked") : signal.deployResult?.error ? "\u274C " + signal.deployResult.error : "\u2705 DEPLOYED";
          return `${i + 1}. ${pool} | wallet #${signal.walletRank} score ${signal.walletScore} | ${(signal.confidence * 100).toFixed(0)}% | ${status}`;
        });
...
function getDeterministicCloseRule(position, managementConfig) {
  ...
  if (
    ...
    // Rule 3: Far above range — close immediately
    _adaptiveDelayMap.delete(position.position);
  ...
  if (
    ...
    // Rule 3: Far below range — close immediately
    _adaptiveDelayMap.delete(position.position);
  ...
  if (
    ...
    // Rule 4: OOR above — check adaptive delay
  ...
  if (
    ...
    // Rule 4: OOR below — check adaptive delay
