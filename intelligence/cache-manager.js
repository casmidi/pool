...
import { redisDel, redisGetJson, redisSetJson } from "./redis-cache.js";
...
export function cacheSet(key, value, opts = {}) {
  ...
  void redisSetJson(key, { value, cachedAt: now }, opts);
}
...
export async function cacheWrap(key, fetcher, opts = {}) {
  ...
  const redisEntry = await redisGetJson(key, opts);
  if (redisEntry && Object.prototype.hasOwnProperty.call(redisEntry, "value")) {
    cacheSet(key, redisEntry.value, opts);
